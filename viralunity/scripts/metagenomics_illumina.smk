...
def _chain_steps(track):
    ...
    if run_nr_validation and track.endswith("contigs"):
        steps.append("nr")
    # Largest-contig size + median depth: contig tracks (each remaps reads to its
    # own viral contigs for a depth BAM), and only when RPKM/--viral-genomes is on.
    if compute_rpkm and track.endswith("contigs"):
...
include: "rules/metagenomics_nr_validation_illumina.smk"
